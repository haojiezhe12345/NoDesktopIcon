...
**A Windows Shell Extension to prevent Desktop icons from loading.**
...
**This shell extension completely prevents the internal loading of desktop icons by patching `CDefView::_ReloadView` in `shell32.dll`.**
...
So our goal is to simply:  
- search for bytes: `8B FA 48 8B D9 BE 01 00 00 00 75`  
- and replace with: `8B FA 48 8B D9 BE 00 00 00 00 EB`

The first 5 bytes are for precise locating, and the jump offset byte is ignored because it varies across different versions of `shell32.dll`.
...
To ensure other programs' integrity, we should only inject to `explorer.exe`.
...
With a well-configured registry, this function will be called at a very early stage of explorer initiation, before Desktop is initiated.  
